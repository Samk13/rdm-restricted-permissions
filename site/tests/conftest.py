...
@contextmanager
def kth_record_permission_policy_active(app):
    """Context manager to ensure record community exists config."""
    try:
        app.config["RDM_PERMISSION_POLICY"] = KTHRecordPermissionPolicy
        yield
    finally:
        app.config["RDM_PERMISSION_POLICY"] = RDMRecordPermissionPolicy
